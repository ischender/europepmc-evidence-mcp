...
This repo is set up so Cursor, Claude Code, and similar tools share one operating
manual.
...
Agents should read `AGENTS.md` first, then `docs/design/`, before implementing
against Europe PMC.
...
- Do not invent a seventh tool without checking whether it is a parameter on an
  existing one.
- Do not stage, commit, or push unless a human explicitly asks.
- Non-trivial features get a living plan under `docs/plans/` before
  implementation (see `AGENTS.md` § Plans).

## Humans reviewing agent work

- Treat `AGENTS.md` as the contract: if agent behaviour drifts, fix the doc.
- Design changes that affect product behaviour belong in `docs/design/`, with a
  short note in any active plan’s Decision Log when open questions are settled.
- This is a normal open source project; keep private / pre-build scratch notes
